document.addEventListener("DOMContentLoaded", () => {
  ...
  let currentPlayer = 1;
  // make a const that stores all possible winning arrays
  const winningArrays = [
    [0, 1, 2, 3],
    [41, 40, 39, 38],
    [7, 8, 9, 10],
  ...
  function checkBoard() {
    ...
    for (let y = 0; y < winningArrays.length; y++) {
      ...
      // now check to see if all the arrays have the player-one class
      if (
      ...
      // now check to see if all the arrays have the player-two class
      else if (
